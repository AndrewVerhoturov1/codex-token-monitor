...
| Field | Format |
|---|---|
| `payload_files[].path` | repo-relative (no `payload/`) |
| `checksums.sha256` | `<sha256>  <path>` (no `payload/`) |
| `context_readback` | repo-relative (no `payload/`) |
| `verification_files[]` | repo-relative (no `payload/`) |
| `metadata.context_readback` | repo-relative (no `payload/`) |
...
The canonical external agent static manual is at `docs/zchat_external_agent_static_manual.md`.
It defines the full agent contract: role, source priority, required reading order, stop-if-missing-information rule,
strict response modes (PACKAGE_READY / BLOCKED_MISSING_CONTEXT / CONTRACT_CONFLICT), ZIP assembly procedure,
path rule, manifest v2 template, checksums template, physical ZIP structure, context readback format,
verification files policy, quarantine-first semantics, Sources Read Report rules, and version reporting.

The canonical repo navigation is at `docs/zchat_repo_navigation.md`. It provides a curated map of what the
external agent may rely on and what it must not assume, plus the request naming convention.
...
docs/                # Public canonical docs (committed to main)
  zchat_external_agent_static_manual.md   # External agent static manual
  zchat_repo_navigation.md               # External agent repo navigation
```

## Request Naming

Canonical format: `ZCHAT-YYYYMMDD-HHMMSS-<slug>`. UTC timestamp with mandatory seconds.
Lowercase slug derived from task description (alphanumeric + hyphens).
Generated by `git_utils.zchat_request_name(task)`.
Validated by `git_utils.zchat_request_name_is_valid(name)`.

## ZCHAT Slug ID

Internal format: `ZCHAT-YYYYMMDD-HHMMSS-<sha256hex8>`. Generated by `git_utils.zchat_slug_id()`. Used for internal operational IDs (import, verify, receive, decision, inspect).
