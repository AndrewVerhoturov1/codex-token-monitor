...
## Route C Round-Robin Profile

Параметр `route_c_profile` позволяет активировать строгий round-robin профиль для Route C задач.
По умолчанию `route_c_profile` пуст, и поведение не меняется (используются стандартные `provider_id`/`model_id`).

### Пример конфигурации в opencode_job_defaults.json

```json
"route_c_profile": "",
"route_c_profiles": {
  "ollama_5_round_robin": {
    "state_file": "_local/codex-token-monitor/route_c_round_robin_state.json",
    "accounts": [
      {"account_id": "ollama1", "provider_id": "ollama", "model_id": "MODEL_ID_PLACEHOLDER"},
      {"account_id": "ollama2", "provider_id": "ollama", "model_id": "MODEL_ID_PLACEHOLDER"},
      {"account_id": "ollama3", "provider_id": "ollama", "model_id": "MODEL_ID_PLACEHOLDER"},
      {"account_id": "ollama4", "provider_id": "ollama", "model_id": "MODEL_ID_PLACEHOLDER"},
      {"account_id": "ollama5", "provider_id": "ollama", "model_id": "MODEL_ID_PLACEHOLDER"}
    ]
  }
}
```
...
1. `route_c_profile` задаёт имя активного профиля (например, `"ollama_5_round_robin"`).
2. Профиль берётся из `route_c_profiles` — каждый профиль содержит `state_file` (путь к файлу состояния round-robin) и `accounts` (список аккаунтов с `account_id`, `provider_id`, `model_id`).
3. Перед построением команды адаптера `reserve_next_account()` атомарно читает текущий индекс из state-файла, выбирает следующий аккаунт по кругу, инкрементирует индекс и пишет обратно.
4. `provider_id` и `model_id` из зарезервированного аккаунта подставляются в команду вместо дефолтных.
5. Метаданные резервации (`route_c_profile`, `route_c_profile_account_id`, `route_c_profile_account_index`) возвращаются в MCP-ответе.

State-файл — это JSON с полем `last_index` (последний использованный индекс, по умолчанию -1). Файл создаётся автоматически при первом вызове.

Для защиты от одновременных резерваций из параллельных Route C процессов используется cross-process file lock (через `msvcrt.locking` на Windows / `fcntl.flock` на POSIX). Эксклюзивная блокировка удерживается на отдельный `*.lock`-файл (рядом с state-файлом) на всё время чтения-изменения-записи, что сериализует конкурирующие reservation-вызовы.

### Активация через MCP

```python
opencode_job_run_and_wait(task_text="...", route_c_profile="ollama_5_round_robin")
```

При пустом `route_c_profile` никакого round-robin не происходит — используются дефолтные `provider_id`/`model_id` из конфига.
