...
description: Use when the user explicitly requests Kimi, or when `opencode-mcp-windows-control` Route C deliberately selects the Kimi runtime route for a bounded medium-code task that must use temporary `OPENCODE_CONFIG_CONTENT` without changing `C:\Users\andre\.config\opencode\opencode.jsonc`.
---
...
- `opencode-mcp-windows-control` Route C deliberately selects the Kimi runtime route.

This skill is conditional.

Kimi is not the default OpenCode route.
Kimi is not automatic fallback after DeepSeek failure.
Kimi is a deliberate bounded medium-code route.

Use temporary `OPENCODE_CONFIG_CONTENT`.
Do not change permanent [opencode.jsonc](</C:/Users/andre/.config/opencode/opencode.jsonc>).

## When Route C is selected by phase planning

This skill is used when the main OpenCode routing skill selects Route C for a
specific implementation phase.

Route C does not have to own the whole user task.

Normal route chain:

- Route A gathers context;
- Route C implements the medium bounded code change;
...
- [scripts/run-opencode-kimi-runtime.ps1](</C:/Users/andre/.codex/skills/opencode-kimi-runtime-launch/scripts/run-opencode-kimi-runtime.ps1>)
...
If Route C / Kimi runtime is already deliberately selected and the health
check fails because local FreeGLMKimiAPI appears to be not running, Codex
may perform one bounded diagnostic bootstrap attempt.
...
powershell -ExecutionPolicy Bypass -File `
  C:\Users\andre\.codex\skills\opencode-kimi-runtime-launch\scripts\run-opencode-kimi-runtime.ps1 `
...
```text
Kimi is deliberate, bounded, and temporary-config only.
Kimi is not default.
